...
router.get('/details/:isbn', async (req, res) => {
  ...
  try {
    ...
    let response = await axios.get(
      `https://www.googleapis.com/books/v1/volumes?q=${isbn}&intitle:${first.data.items[0].volumeInfo.title}&maxResults=40&key=${process.env.GOOGLE_KEY}`
    );
    // res.send(response.data.items.filter((book) => book.saleInfo.saleability === ‘FOR_SALE’)[0]);
    res.send(
      response.data.items.filter((book) => {
