...
//   try {
//     nytSelectedLists.forEach(listName => {
//       await axios.get(`https://api.nytimes.com/svc/books/v3/lists/current/${listName}.json`,
//         { params: { api-key: '0u7qPMbjUFiEb2b8zkQlBRo8TSsk2Avm'}, body: { list: listName}})
//           .then(topTitles => {
//             selectBestSellers.push.topTitles.data.results.books[0].primary_isbn10;
//             })
//           })
//           .catch(error => {
...
const getRecommendedBooks = async () => {
  ...
  try {
    nytSelectedLists.forEach(async (listName) => {
      await axios.get('/book/bestsellers')
      // { params: { list: listName, apikey: '0u7qPMbjUFiEb2b8zkQlBRo8TSsk2Avm' }})
        .then(topTitles => {
          selectBestSellers.push.topTitles.data.results.books[0].primary_isbn10;
        })
        .catch(error => {
